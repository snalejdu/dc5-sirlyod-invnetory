  ...
  <div class="container py-4">
    ...
    <form @submit.prevent="submit" class="bg-white p-4 rounded shadow" style="max-width: 600px;">
      ...
      <div class="mb-3">
        ...
        <textarea v-model="form.description" rows="3" class="form-control" :class="{ 'is-invalid': form.errors.description }" />
        <div class="invalid-feedback">{{ form.errors.description }}</div>
      </div>
      <div class="mb-3">
        <label class="form-label fw-semibold">Price</label>
        <input v-model="form.price" type="number" step="0.01" class="form-control" :class="{ 'is-invalid': form.errors.price }" />
...
const form = useForm({ name: '', sku: '', description: '', price: '', supplier_id: '' })
