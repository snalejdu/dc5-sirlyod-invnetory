<template>
  <div class="container py-4">
    <h1 class="fw-bold mb-4">Create Supplier</h1>
    <form @submit.prevent="submit" class="bg-white p-4 rounded shadow" style="max-width: 600px;">
      ...
      <div class="mb-3">
        ...
        <input v-model="form.phone" type="text" class="form-control" :class="{ 'is-invalid': form.errors.phone }" required />
        <div class="invalid-feedback">{{ form.errors.phone }}</div>
      </div>
      <div class="mb-3">
        <label class="form-label fw-semibold">Address</label>
        <textarea v-model="form.address" rows="3" class="form-control" :class="{ 'is-invalid': form.errors.address }" required></textarea>
        <div class="invalid-feedback">{{ form.errors.address }}</div>
      </div>
      <div class="mb-3">
        <label class="form-label fw-semibold">Contact Person</label>
        <input v-model="form.contact_person" type="text" class="form-control" :class="{ 'is-invalid': form.errors.contact_person }" required />
